...
import { UPSData, SystemNode } from '../types';
...
const CommandDeck: React.FC<Props> = ({ data, enableAudibleAlarms }) => {
  ...
  const renderModalContent = () => {
    ...
    switch (selectedNode) {
        ...
        case 'BATTERY':
            // Calculate a Software Estimate (conservative 85% of UPS reported)
            const swRuntime = Math.floor(data.runtimeRemaining * 0.85);
            
            // Logic for Charging/Discharging status
  ...
  return (
    ...
    `}>
      ...
      {/* Middle Section - Flex container that scrolls on mobile, fits on desktop */}
      <div className="flex flex-col md:flex-1 md:min-h-0 items-center justify-center relative gap-4 md:gap-0">
                ...
                >
                    <div className="bg-red-950/40 border-2 border-red-600 rounded-lg p-2 md:p-4 flex flex-col items-center justify-center shadow-[0_0_50px_rgba(220,38,38,0.3)] backdrop-blur-sm relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 text-red-500 mb-1 relative z-10">
                            ...
                            <span className="text-xs md:text-sm font-mono font-bold tracking-[0.2em] uppercase">Time Remaining</span>
                        </div>
        ...
        {/* Visualizer Row - Responsive Layout */}
        <div className="w-full flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-8 md:flex-1 md:min-h-0">
            ...
            {/* Power Stream - The flexible centerpiece */}
            <div className="w-full h-[250px] md:h-full md:flex-1 md:min-h-0 min-w-0 relative flex flex-col items-center justify-center">
                ...
                <div className="shrink-0 pt-2 text-center text-[9px] text-gray-500 font-mono animate-pulse hidden md:block pointer-events-none">
                    CLICK NODES FOR TELEMETRY
                </div>
            </div>
      ...
      <div className="shrink-0 grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 border-t border-gray-800 pt-4 mt-auto md:mt-0">
        <MetricCard 
            label="BATTERY TEMP" 
            value={`${data.batteryTemp}°C`} 
            color={isOverheat ? COLORS.red : data.batteryTemp > 35 ? COLORS.orange : COLORS.cyan} 
            icon={<IconTemp />}
            className={isOverheat ? 'animate-pulse border-red-500 bg-red-900/30' : ''}
        />
        <MetricCard 
            label="OUTPUT AMPS" 
            value={`${data.outputAmps.toFixed(2)}A`} 
            icon={<IconAmps />}
        />
        <MetricCard 
            label="REAL POWER" 
            value={`${data.realPowerW.toFixed(2)}W`} 
            icon={<IconZap />}
        />
        <MetricCard 
            label="RUNTIME EST." 
            value={`${Math.floor(data.runtimeRemaining/60)}m`} 
            icon={<IconClock />}
        />
      </div>
        ...
        {selectedNode && (
            <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/60 p-4">
                <motion.div 
                    ...
                    className="bg-black border border-neon-cyan p-6 md:p-8 rounded-lg shadow-[0_0_50px_rgba(0,240,255,0.1)] w-full max-w-md relative"
                >
                    <button 
...
const MetricCard: React.FC<{ label: string, value: string, color?: string, icon?: React.ReactNode, className?: string }> = ({ label, value, color = COLORS.cyan, icon, className = '' }) => (
    <div className={`bg-gray-900/50 p-2 md:p-3 rounded border border-gray-800 flex items-center justify-between hover:border-neon-cyan/50 transition-colors group ${className}`}>
        ...
        {icon && (
            <div className="text-gray-700 group-hover:text-neon-cyan transition-colors transform scale-75 md:scale-90 shrink-0">
                {icon}
            </div>
        )}
    </div>
